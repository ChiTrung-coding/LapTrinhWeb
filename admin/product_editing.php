...
<div class="main-content">
    <h1><?= !empty($_GET['id']) ? ((!empty($_GET['task']) && $_GET['task'] == "copy") ? "Copy sản phẩm" : "Sửa sản phẩm") : "Thêm sản phẩm" ?>
    </h1>
    <div id="content-box">
        <?php
            if (isset($_GET['action']) && ($_GET['action'] == 'add' || $_GET['action'] == 'edit')) {
                ...
                && isset($_POST['quantity']) && !empty($_POST['quantity'])) {
                    ...
                    }elseif (!empty($_POST['quantity']) && is_numeric(str_replace('.', '', $_POST['quantity'])) == false) {
                        $error = "Nhập số lượng không hợp lệ!";
                    }
                    ...
                    if (!isset($error)) {
                        if ($_GET['action'] == 'edit' && !empty($_GET['id'])) { //Cập nhật lại sản phẩm
                            $result = mysqli_query($con, "UPDATE `product` SET `name` = '" . $_POST['name'] . "',`image` =  '" . $image . "', `price` = " . str_replace('.', '', $_POST['price']) . ", `quantity` = " . str_replace('.', '', $_POST['quantity']) . ", `content` = '" . $_POST['content'] . "', `last_updated` = " . time() . " WHERE `product`.`id` = " . $_GET['id']);
                        } else { //Thêm sản phẩm
                            $result = mysqli_query($con, "INSERT INTO `product` (`id`, `name`, `image`, `price`,`quantity`, `content`, `created_time`, `last_updated`) VALUES (NULL, '" . $_POST['name'] . "','" . $image . "', " . str_replace('.', '', $_POST['price']) . ", " . str_replace('.', '', $_POST['quantity']) . " ,'" . $_POST['content'] . "', " . time() . ", " . time() . ");");
                        }
        ...
        <form id="editing-form" method="POST"
            ...
            <div class="wrap-field">
                ...
                <input type="text" name="quantity"
                    value="<?= (!empty($product) ? number_format($product['quantity'], 0, ",", ".") : "") ?>" />
                <div class="clear-both"></div>
            </div>
            <div class="wrap-field">
